import sys
import os
import numpy as np
import pandas as pd
import multiprocess as mp
from pathlib import Path

# --- Data Loading and Preprocessing (Unchanged) ---
REPO_ROOT = Path(__file__).resolve().parents[1]
DATA_DIR = REPO_ROOT / 'data' / 'benchmarks'
OUTPUT_DIR = REPO_ROOT / 'results' / 'applications'
binary_df_math500 = pd.read_csv(DATA_DIR / 'correctness_matrix_math500.csv', index_col=0)
cot_df_math500 = pd.read_csv(DATA_DIR / 'cot_length_matrix_math500.csv', index_col=0)
binary_df_aime24 = pd.read_csv(DATA_DIR / 'correctness_matrix_aime24.csv', index_col=0)
cot_df_aime24 = pd.read_csv(DATA_DIR / 'cot_length_matrix_aime24.csv', index_col=0)
binary_df_aime25 = pd.read_csv(DATA_DIR / 'correctness_matrix_aime25.csv', index_col=0)
cot_df_aime25 = pd.read_csv(DATA_DIR / 'cot_length_matrix_aime25.csv', index_col=0)
binary_df_amc23 = pd.read_csv(DATA_DIR / 'correctness_matrix_amc23.csv', index_col=0)
cot_df_amc23 = pd.read_csv(DATA_DIR / 'cot_length_matrix_amc23.csv', index_col=0)

sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

from cMIRT_EM_c import cMIRT_SAEM_full, MIRT_SAEM_full

rows_to_delete = [
    "meta_llama_Llama_3.2_1B_one_shot",
    "meta_llama_Llama_3.2_1B_zero_shot",
    "meta_llama_Meta_Llama_3_8B_one_shot",
    "meta_llama_Meta_Llama_3_8B_zero_shot",
    "microsoft_phi_4_one_shot",
    "microsoft_phi_4_zero_shot",
    'TinyLlama_TinyLlama_1.1B_Chat_v1.0_zero_shot',
    'TinyLlama_TinyLlama_1.1B_Chat_v1.0_one_shot',
    'google_gemma_3_1b_pt_one_shot',
    'google_gemma_3_1b_pt_zero_shot',
    'google_gemma_7b_it_one_shot',
    'google_gemma_7b_it_zero_shot',
    'google_vaultgemma_1b_one_shot',
    'google_vaultgemma_1b_zero_shot',
    'meta_llama_Llama_3.2_3B_one_shot',
    'meta_llama_Llama_3.2_3B_zero_shot',
    'openai_community_gpt2_one_shot',
    'openai_community_gpt2_zero_shot',
    "google_gemma_2_27b_it_one_shot",
    "google_gemma_2_27b_it_zero_shot"
]

binary_df_math500 = binary_df_math500.drop(rows_to_delete, errors='ignore')
cot_df_math500 = cot_df_math500.drop(rows_to_delete, errors='ignore')
binary_df_aime24 = binary_df_aime24.drop(rows_to_delete, errors='ignore')
cot_df_aime24 = cot_df_aime24.drop(rows_to_delete, errors='ignore')
binary_df_aime25 = binary_df_aime25.drop(rows_to_delete, errors='ignore')
cot_df_aime25 = cot_df_aime25.drop(rows_to_delete, errors='ignore')
binary_df_amc23 = binary_df_amc23.drop(rows_to_delete, errors='ignore')
cot_df_amc23 = cot_df_amc23.drop(rows_to_delete, errors='ignore')

binary_df_math500.index = binary_df_math500.index.str.lower()
cot_df_math500.index = cot_df_math500.index.str.lower()
binary_df_aime24.index = binary_df_aime24.index.str.lower()
binary_df_aime25.index = binary_df_aime25.index.str.lower()
binary_df_amc23.index = binary_df_amc23.index.str.lower()
cot_df_aime24.index = cot_df_aime24.index.str.lower()
cot_df_aime25.index = cot_df_aime25.index.str.lower()
cot_df_amc23.index = cot_df_amc23.index.str.lower()

binary_all = pd.concat([binary_df_math500, binary_df_aime24, binary_df_aime25, binary_df_amc23], axis=1)
cot_all = pd.concat([cot_df_math500, cot_df_aime24, cot_df_aime25, cot_df_amc23], axis=1)

binary_array = binary_all.to_numpy()
cot_array = cot_all.to_numpy()
cot_array += 1

N, J = binary_array.shape

# --- Model Selection (Unchanged) ---
np.random.seed(42)
chosen_models = np.random.choice(range(N), 108, replace=False)
chosen_models_bool = np.zeros(N, dtype=bool)
chosen_models_bool[chosen_models] = True


# --- Main Execution Block for Parallel Processing ---
if __name__ == "__main__":
    # Create a pool with 2 processes to run both models at the same time
    with mp.Pool(processes=2) as pool:
        print("Starting cMIRT and MIRT model fitting in parallel...")

        # 1. Submit the first task (cMIRT) to the pool
        cmirt_result_async = pool.apply_async(
            cMIRT_SAEM_full,
            args=(binary_array[chosen_models_bool, :], cot_array[chosen_models_bool, :]),
            kwds={'n_samples': 1, 'seed': 42}
        )

        # 2. Submit the second task (MIRT) to the pool
        mirt_result_async = pool.apply_async(
            MIRT_SAEM_full,
            args=(binary_array[chosen_models_bool, :],),
            kwds={'n_samples': 1, 'seed': 42}
        )

        # 3. Wait for the results from both tasks
        print("Waiting for results...")
        theta_joint, tau_joint, a_joint, b_joint, omega_joint, phi_joint, lam_joint, rho_joint, n_iter_joint = cmirt_result_async.get()
        theta_irt, a_irt, b_irt, sigma2_irt, n_iter_irt = mirt_result_async.get()

    print("Model fitting complete. Saving results...")

    # 4. Save the estimated parameters into a single parquet file
    # The arrays are wrapped in lists to be stored correctly in a DataFrame
    results_df = pd.DataFrame({
        'chosen_models': [chosen_models.tolist()],
        'theta_joint': [theta_joint],
        'tau_joint': [tau_joint],
        'a_joint': [a_joint],
        'b_joint': [b_joint],
        'omega_joint': [omega_joint],
        'phi_joint': [phi_joint],
        'lam_joint': [lam_joint],
        'rho_joint': [rho_joint],
        'theta_irt': [theta_irt],
        'a_irt': [a_irt],
        'b_irt': [b_irt],
        'sigma2_irt': [sigma2_irt],
        'n_iter_joint': n_iter_joint,
        'n_iter_irt': n_iter_irt
    })

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    output_path = OUTPUT_DIR / 'estimated_parameters_all_pred.parquet'
    results_df.to_parquet(output_path, index=False)
    print(f"Results saved to '{output_path}'")
