...
def run_model_task(task_tuple, binary_data, cot_data):
    ...
    try:
        if model_type == 'LaRT':
            ...
            # Run the IRT model
            (theta_irt, a_irt, b_irt, n_iter_irt) = IRT_SAEM_full(
                binary_data[:N, :],
                n_samples=1, seed=42
            )
            ...
            return {
                ...
                'b_irt': [b_irt], 'n_iter_irt': n_iter_irt
            }
...
if __name__ == "__main__":
    ...
    results_df.to_parquet('efficiency_llm_math500.parquet', index=False)
    print("Results saved to 'efficiency_llm_math500.parquet'")