"""Traditional burn-in SAEM variant used only for Appendix E.3."""
...
def post_samp_tau_theta(n_samples, R, log_T, a, b, omega, phi, lam, rho, seed = None):
    ...
    for i in range(N):
        ...
        #            mu_t au_reorg/sigma_tau2_reorg) * sigma_tau2_p
...
def cMIRT_SAEM(R, T, n_samples=1, weight_scheme_fn = None, initial_params = None, n_burn=60, eps = 1e-4, max_iter=100, seed=None):
    ...
    a, b, omega, phi, lam, rho, loss_old, grad_old = update_all_given_indi_samp(theta_samples, tau_samples, R, log_T, a_init, b_init, omega_init, phi_init, lam_init, rho_init,
                                                                           weight=1, loss_old=None, grad_old=None)
    # lam = np.abs(lam)
    ...
    #     if seed is not None:
    #         seed += 1
    #     theta_samples, tau_samples = post_samp_tau_theta(current_n_samples, R, log_T, a_init, b_init, omega_init, phi_init, lam_init, rho_init, seed=seed)
    #     a, b, omega, phi, lam, rho, loss_old, grad_old = update_all_given_indi_samp(theta_samples, tau_samples, R, log_T, a_init, b_init, omega_init, phi_init, lam_init, rho_init,
    ...
    while (err > eps) and (n_iter < max_iter):
        ...
        if n_iter <= n_burn:
            ...
            a_new, b_new, omega_new, phi_new, lam_new, rho_new, loss_old, grad_old = update_all_given_indi_samp(theta_samples, tau_samples, R, log_T, a, b, omega, phi, lam, rho,
                                                                                                         weight=current_weight, loss_old=None, grad_old=None)
        else:
            current_weight = weight_scheme_fn(n_iter - n_burn)
            a_new, b_new, omega_new, phi_new, lam_new, rho_new, loss_old, grad_old = update_all_given_indi_samp(theta_samples, tau_samples, R, log_T, a, b, omega, phi, lam, rho,
                                                                                                         weight=current_weight, loss_old=loss_old, grad_old=grad_old)
        # lam = np.abs(lam)
...
def MIRT_SAEM_full(R, n_samples = 10, initial_params = None, weight_scheme_fn = None,
    ...
    while (err > eps) and (n_iter < max_iter):
        ...
        if n_iter <= n_burn:
            ...
            current_weight = weight_scheme_fn(n_iter - n_burn)
            a_new, b_new, sigma2_new, loss_fn, grad_fn = update_all_given_indi_samp(theta_samples, R, a, b, sigma2,
                                      weight=current_weight, loss_old=loss_fn, grad_old=grad_fn)
