"""Reference SAEM implementation used for the LaRT paper experiments.

The historical function names are retained so that the original experiment
scripts remain reproducible. New users should normally call :func:`lart.fit_lart`
or :func:`lart.fit_irt`.
"""
...
def cMIRT_SAEM(R, T, n_samples=1, weight_scheme_fn = None, initial_params = None, eps = 1e-4, max_iter=100, seed=None):
    log_T = np.log(T)
    ...
    # lam = np.abs(lam)
    # log_like = log_complete_like_samp(R, log_T, theta_samples, tau_samples, a, b, omega, phi, lam, Sigma)
    ...
    while (err > eps) and (n_iter < max_iter):
        ...
        # lam = np.abs(lam)
        # log_like_new = log_complete_like_samp(R, log_T, theta_samples, tau_samples, a_new, b_new, omega_new, phi_new, lam_new, Sigma_new)
        ...
        #                                                                                                      weight=weight_scheme_fn(n_iter), loss_old=loss_old, grad_old=grad_old)
        #     err = np.mean(np.abs(a_new - a)) + np.mean(np.abs(b_new - b))
        # + np.mean(np.abs(omega_new - omega)) + \
        #     np.mean(np.abs(phi_new - phi)) + np.mean(np.abs(lam_new - lam)) + np.mean(np.abs(Sigma_new - Sigma))
...
def cMIRT_SAEM_full(R, T, n_samples=1, weight_scheme_fn = None, initial_params = None, eps = 1e-4, max_iter=100, seed=None):
    ...
    a_est, b_est, omega_est, phi_est, lam_est, rho_est, n_iter = cMIRT_SAEM(R, T, n_samples, weight_scheme_fn, initial_params, eps, max_iter, seed=seed)
...
def MIRT_SAEM_full(R, n_samples = 1, initial_params = None, weight_scheme_fn = None,
    ...
    while (err > eps) and (n_iter < max_iter):
        ...
        # log_like_new = log_c_like_samp(R, theta_samples, a_new, b_new, sigma2_new)
        # if log_like_new > log_like:
        #     log_like = log_like_new
        #     a_est, b_est, sigma2_est = a_new, b_new, sigma2_new
        err = np.mean(np.abs(a_new - a)) + np.mean(np.abs(b_new - b))
        # if err > 1:
        #     if seed is not None:
        #         seed += 1
        #     theta_samples = post_samp_theta(current_n_samples, R, a, b, seed=seed)
        #     a_new, b_new, sigma2_new, loss_fn, grad_fn = update_all_given_indi_samp(theta_samples, R, a, b, sigma2,
        #                                   weight_scheme_fn(n_iter), loss_old=loss_fn, grad_old=grad_fn)
        #     err = np.mean(np.abs(a_new - a)) + np.mean(np.abs(b_new - b))
        # + np.abs(sigma2_new - sigma2)
