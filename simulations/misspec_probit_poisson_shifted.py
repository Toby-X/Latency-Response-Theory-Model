...
# ---------------------------------------------------------------------------
# Misspecification Scenario 2: Probit-Log-Poisson
# ---------------------------------------------------------------------------
# DATA generated with:
#   Response :  P(R=1) = Phi(theta * a + b)        [probit, same as model]
#   Latency  :  T ~ Poisson(mu_ij)                 [Poisson]
#               mu_ij = exp(omega_j - phi_j*tau_i + log(800))
#               so that E[T] ≈ 800, with person/item-driven variation
#
# MODEL FITTED (LaRT) assumes:
#   Response :  P(R=1) = Phi(theta * a + b)         [probit] -- correct
#   Latency  :  log(T) ~ N(omega - phi*tau, lam)    [log-normal] <-- misspecified
#
...
N_DEFAULT      = 100
J_DEFAULT      = 50
OFFSET_800 = 800   # baseline offset so E[mu] ≈ 800


def gen_indi_given_all_probit_poisson(N, a, b, omega, phi, lam, Sigma, seed=None):
    """
    Generate responses via probit link and latencies via Poisson.
    lam is accepted for API compatibility but not used in Poisson generation.
    """
    ...
    # Probit response (correct link)
    probit_arg = theta[:, np.newaxis] * a[np.newaxis, :] + b[np.newaxis, :]
    prob = norm.cdf(probit_arg)
    R    = rng.binomial(1, prob)

    # Poisson latency (misspecification)
    # log-rate: omega_j - phi_j * tau_i + offset to push mean to ~800
    mu = omega[np.newaxis, :] - phi[np.newaxis, :] * tau[:, np.newaxis] + OFFSET_800
    # mu = np.exp(log_rate)                             # shape (N, J); E[T_ij] ≈ 800
    T  = rng.poisson(mu).astype(float) + 1
    # Avoid log(0) when the model later takes log(T)
...
def gen_data(N, J, seed=None):
    """Generate synthetic data (probit-Poisson) with N subjects, J items."""
    rng = np.random.default_rng(seed)

    a     = rng.uniform(0.5, 1,   size=J)
    b     = rng.normal(0, 0.5,    size=J)
    omega = rng.normal(0, 40,      size=J)
    phi   = rng.uniform(30, 70, size=J)
    lam   = rng.uniform(0.5, 2,   size=J)   # kept for signature compatibility
    Sigma = np.array([[1., -.8], [-.8, 1.]])

    R, T, theta, tau = gen_indi_given_all_probit_poisson(
...
def experiment_fn(seed, param_dict, esp=1e-4, max_iter=100):
    """Single replicate: probit-Poisson data, probit-log-normal model."""
    try:
        np.random.seed(seed)
        ...
        return {
            'N':                   N,
            'J':                   J,
            'misspecification':    'probit_poisson',
...
def run_experiment(param_grid, n_exp=200, n_cores=8):
    ...
    with mp.Pool(processes=n_cores) as pool:
        results = list(tqdm(
            ...
            total=len(all_exp),
            desc="Misspec: probit-log-Poisson",
        ))

    results    = [r for r in results if r is not None]
    results_df = pd.DataFrame(results)

    out_file = save_results(results_df, "cMIRT_misspec_probit_poisson_lin.parquet")
