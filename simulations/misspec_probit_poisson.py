...
N_DEFAULT      = 100
J_DEFAULT      = 50
LOG_800_OFFSET = np.log(800)   # baseline offset so E[mu] ≈ 800
...
def gen_indi_given_all_probit_poisson(N, a, b, omega, phi, lam, Sigma, seed=None):
    ...
    log_rate = (omega[np.newaxis, :] - phi[np.newaxis, :] * tau[:, np.newaxis]
                + LOG_800_OFFSET)
...
def run_experiment(param_grid, n_exp=200, n_cores=8):
    ...
    out_file = save_results(results_df, "cMIRT_misspec_probit_poisson.parquet")
