...
# ---------------------------------------------------------------------------
# Misspecification Scenario 3: Logistic-Log-Poisson
# ---------------------------------------------------------------------------
# DATA generated with:
...
#   Response :  P(R=1) = Phi(theta * a + b)         [probit]    <-- misspecified
...
def gen_indi_given_all_logistic_poisson(N, a, b, omega, phi, lam, Sigma, seed=None):
    """
    Generate responses via logistic link and latencies via Poisson.
    lam is accepted for API compatibility but is not used in Poisson generation.
    """
...
def gen_data(N, J, seed=None):
    """Generate synthetic data (logistic-Poisson) with N subjects, J items."""
    rng = np.random.default_rng(seed)
    ...
    R, T, theta, tau = gen_indi_given_all_logistic_poisson(
...
def experiment_fn(seed, param_dict, esp=1e-4, max_iter=100):
    """Single replicate: logistic-Poisson data, probit-log-normal model."""
    try:
        np.random.seed(seed)
        ...
        return {
            'N':                   N,
            'J':                   J,
            'misspecification':    'logistic_poisson',
...
def run_experiment(param_grid, n_exp=200, n_cores=8):
    ...
    with mp.Pool(processes=n_cores) as pool:
        results = list(tqdm(
            ...
            total=len(all_exp),
            desc="Misspec: logistic-log-Poisson",
        ))

    results    = [r for r in results if r is not None]
    results_df = pd.DataFrame(results)

    out_file = save_results(results_df, "cMIRT_misspec_logistic_poisson.parquet")
