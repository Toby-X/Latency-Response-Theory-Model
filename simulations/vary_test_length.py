...
# ---------------------------------------------------------------------------
# Simulation: Fixed N, Increasing J
# ---------------------------------------------------------------------------
# N is held constant at N_FIXED.  J ranges over J_GRID.
# Each (J, seed) combination is an independent replication.
# ---------------------------------------------------------------------------

N_FIXED = 200
J_GRID  = [20, 50, 100, 200]
...
def experiment_fn(seed, param_dict, esp=1e-4, max_iter=100):
    """Single experiment: fixed N, draw J from param_dict."""
    try:
        np.random.seed(seed)
        ...
        print(f"Error in experiment (seed={seed}, J={param_dict.get('J')}): {e}")
        return None
...
def run_experiment(param_grid, n_exp=200, n_cores=8):
    ...
    with mp.Pool(processes=n_cores) as pool:
        results = list(tqdm(
            ...
            total=len(all_exp),
            desc=f"Running experiments (fixed N={N_FIXED}, varying J)",
        ))

    results    = [r for r in results if r is not None]
    results_df = pd.DataFrame(results)

    out_file = save_results(results_df, f"cMIRT_sim_fixedN{N_FIXED}_varyJ.parquet")
...
if __name__ == "__main__":
    ...
    print(results_df.groupby('J')[['rmse_theta', 'rmse_tau', 'rmse_a', 'rmse_b',
