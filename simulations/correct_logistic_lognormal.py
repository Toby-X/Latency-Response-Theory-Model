...
def gen_indi_given_all_logistic_lognormal(N, a, b, omega, phi, lam, Sigma, seed=None):
    ...
    # Log-normal latency (correctly specified)
    # T = exp( omega - phi*tau + N(0, lam^2) )
    log_T_mean = omega[np.newaxis, :] - phi[np.newaxis, :] * tau[:, np.newaxis]
    noise      = rng.normal(0, lam[np.newaxis, :], size=(N, len(a)))
    log_T      = log_T_mean + noise
    T          = np.exp(log_T)
...
def gen_data(N, J, seed=None):
    """Generate synthetic data (logistic-lognormal) with N subjects, J items."""
    rng = np.random.default_rng(seed)
...
def experiment_fn(seed, param_dict, esp=1e-4, max_iter=100):
    """Single replicate: logistic-lognormal data, probit-log-normal model."""
    try:
        np.random.seed(seed)
        ...
        estimates = twopl_mml(R.T)
        a_est_irt = estimates['Discrimination']
        diff_est_irt = estimates['Difficulty']

        # Convert difficulty back to intercept format: b = -a * difficulty
        b_est_irt = -a_est_irt * diff_est_irt

        # Calculate the Expected A Posteriori (EAP) for theta
        theta_est_irt = ability_eap(R.T, diff_est_irt, a_est_irt)

        iter_irt = -1 # dummy value
        ...
        return {
            ...
            # IRT rank correlations (from girth)
